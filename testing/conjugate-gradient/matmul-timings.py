...
for N in N_range:
    K_full = get_stiffness(N)
    f_full = get_force(N)

    K = spsp.csr_array(K_full)
...
plt.figure()
plt.loglog(N_range[:len(time1_list)], time1_list, label=r'$K_{s} @ f_{f}$')
plt.loglog(N_range[:len(time2_list)], time2_list, label=r'$K_{s} @ f_{s}$')
plt.loglog(N_range[:len(time3_list)], time3_list, label=r'$K_{s} @ f_{s}.T$')
plt.loglog(N_range[:len(time4_list)], time4_list, label=r'$K_{s} @ f_{s}.tranpose()$')
plt.legend()
plt.show()
