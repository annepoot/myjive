...
for load in ['forced displacement', 'point load']:
    for direction in ['horizontal', 'vertical', 'poisson']:
        for loc in [1, 3, 5, 7, 9]:
            for prior in ['M', 'K']:
                for prior_mean in ['zero', 'nonzero']:
                    ...
                    if prior_mean == 'nonzero':
                        if load == 'forced displacement':
                            prior_mean = 'dirichlet'
                        elif load == 'point load':
                            prior_mean = 'neumann'

                    props['gpsolver']['priorMean'] = prior_mean
                    ...
                    contraction = abs(Sigma_post @ np.linalg.solve(Sigma_prior, u))

                    std_u_post_proj = np.linalg.solve(Phi.T @ Phi, Phi.T @ std_u_post)
                    std_u_post_sub = Phi_sub.T @ std_u_post

                    info = '{}-prior, {}, {}, {}-mean, hole {}'.format(prior, load, direction, prior_mean, loc)
                    ...
                    if model == 'solid':
                        ...
                        QuickViewer(contraction, globdat, comp=0, ax=ax5, title=r'Posterior contraction ($x$) ($\Sigma_{post} \Sigma_{prior}^{-1} u_f$)')
                        fig.suptitle(info)
                        ...
                        QuickViewer(contraction, globdat, comp=1, ax=ax5, title=r'Posterior contraction ($y$) ($\Sigma_{post} \Sigma_{prior}^{-1} u_f$)')
                        fig.suptitle(info)
                        ...
                        QuickViewer(contraction, globdat, comp=0, ax=ax5, title=r'Posterior contraction ($u$) ($\Sigma_{post} \Sigma_{prior}^{-1} u_f$)')
                        fig.suptitle(info)
