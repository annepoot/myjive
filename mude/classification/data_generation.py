...
risk_rule = (
    "'unnecessary' if n_det < 8 else 'maintenance' if n_det < 16 else 'demolition'"
)
...
for sample in range(nsamples):
    ...
    cwd = os.getcwd()
    os.replace(
        cwd + "/stiffness1.vtu", cwd + "/vtk-output/stiffness{}.vtu".format(sample + 1)
    )
...
with open("README.txt", "w") as f:
    ...
    f.write(
        "E ~ lognormal(mean: ln({:.0f}), std: {:.1f})\n".format(np.exp(E_mean), E_std)
    )
    f.write(
        "nu ~ lognormal(mean: ln({:.1f}), std: {:.1f})\n".format(
            np.exp(nu_mean), nu_std
        )
    )
    f.write("n_det = {:s}\n".format(n_det_rule))
