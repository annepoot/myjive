...
beta_list    = [ 1e-4, 1e0, 1e2, 1e0, 1e2 ]
epsilon_list = [ 1e-3, 1e0, 1e2, 1e-3, 1e0 ]
label_list   = [ 1, 2, 3, 4, 5 ]

fig,ax=plt.subplots(1,1)
cp = ax.contour(Beta_rel, Epsilon_rel, Error_rel, levels=[1e-10,1e-8,1e-6,1e-4,1e-2,1e-1,5e-1] ,norm = LogNorm())
...
ax.plot(beta_list, epsilon_list, 'k.')
for i, label in enumerate(label_list):
    ax.annotate(label, (beta_list[i], epsilon_list[i]))
