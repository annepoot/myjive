...
props_c['init']['mesh']['file'] = 'meshes/4-node/bar_coarse.msh'
...
QuickViewer(u_post, globdat, comp=0, title=r'Posterior mean diplacement ($\bar u$)')

QuickViewer(u, globdat, comp=0, title=r'Exact displacement ($u$)')

fig, (ax1, ax2) = plt.subplots(2, 1, tight_layout=True)
QuickViewer(err, globdat, comp=0, ax=ax1, title=r'Discretization error ($|u_f - u_c|$)')
QuickViewer(std_u_post, globdat, comp=0, ax=ax2, title=r'Posterior standard deviation ($x$) ($\sqrt{\bar \Sigma_{ii}}$)')
plt.show()

QuickViewer(std_u_post, globdat, comp=1, title=r'Posterior standard deviation ($y$) ($\sqrt{\bar \Sigma_{ii}}$)')

for i, sample in enumerate(samples_u_prior.T[:3]):

    QuickViewer(sample, globdat, comp=0, title=r'Prior samples from $u$ (sample {})'.format(i+1))

for i, sample in enumerate(samples_u_post.T[:3]):

    QuickViewer(sample, globdat, comp=0, title=r'Posterior samples from $u$ (sample {})'.format(i+1))
...
for fineness in fine_list:
    ...
    if fineness != 'post':
        ...
        pro['init']['mesh']['file'] = 'meshes/4-node/bar_' + fineness + '.msh'
    ...
    for n, node in enumerate(nodes):
        ...
        if np.isclose(coords[1], 0):
            ...
            u_bottom.append(u[dofs.get_dof(n, 'dx')])

            if fineness == 'post':
                std_u_bottom.append(std_u_post[dofs.get_dof(n, 'dx')])
